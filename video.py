...
class Window(QtGui.QWidget):
    def __init__(self):
        ...
        # control button
        self.button = QtGui.QPushButton(u'选择文件', self)
    ...
    def update(self):
        ...
        self.info.setText(u"%4.2f 秒" % lapse)
    ...
    def handleStateChanged(self, newstate, oldstate):
        ...
        if (newstate != Phonon.LoadingState and newstate != Phonon.BufferingState):
            self.button.setText(u'选择文件')
            if newstate == Phonon.ErrorState:
                source = self.media.currentSource().fileName()
                print (u'错误：不能播放：', source.toLocal8Bit().data())
                print ('  %s' % self.media.errorString().toLocal8Bit().data())


if __name__ == '__main__':
    app = QtGui.QApplication(sys.argv)
    app.setApplicationName(u'视频播放')
    window = Window()
    window.show()
    sys.exit(app.exec_())
